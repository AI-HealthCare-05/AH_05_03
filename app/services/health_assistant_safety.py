...
class HealthAssistantSafetyService:
    ...
    def check_input_safety(messages: list[ChatMessage]) -> HealthAssistantResponse | None:
        ...
        emergency_keywords = ["가슴이 쥐어짜듯", "심한 흉통", "호흡곤란", "호흡 곤란", "숨쉬기 힘들어", "의식 저하", "마비", "심한 출혈"]
    ...
    def validate_response(response: HealthAssistantResponse) -> HealthAssistantResponse:
        ...
        # 응급 상황이 감지되었지만 안전 고지가 누락된 경우 기본 고지문 추가
        if response.emergency_notice and not response.safety_disclaimer:
            response.safety_disclaimer = "본 서비스는 의료 진단이나 처방을 대신하지 않습니다. 이상 징후가 있을 경우 의료진과 상담하세요."
