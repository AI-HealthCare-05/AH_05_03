...
#: 앞날을 재는 지점. **1년 간격 5년까지.**
#:
#: 예전에는 (5, 10) 이었다. 두 점만으로는 "지금 손대면 얼마나 달라지나" 를 물어볼
#: 자리가 없고, 10년은 이 서비스가 쓰는 근거의 사거리를 넘는다 — 기준 위험표는
#: NHANES 단면에서 역산한 것이고 δ 는 7개 주기 사망연계다. 5년까지가 그 근거로
#: 말할 수 있는 범위이고, 그 안을 한 살 간격으로 채우면 기울기가 보인다.
#:
#: 표는 한 살 간격 h(a) 라 1년 지평이 공짜다 — `project` 가 Λ0 를 해마다 누적한다.
...
EXCLUDED_TARGETS: dict[str, str] = {
    ...
    # 2026-09-07 에 번들이 붙은 셋. 셋 다 **가역**이라 illness-death 역산의 전제가
    # 무너진다 — 그 모형은 "한 번 생기면 없어지지 않는다" 를 가정하고 유병률을
    # 발병률로 뒤집는데, 되돌아가는 라벨에서는 순발생률이 아니라 순전이율이 나온다.
    # 대신 유병 곡선("그 나이에 기준을 넘고 있을 확률")이 열 질환 전부에 붙는다.
    "obesity": "가역(체중으로 드나든다). 라벨이 BMI 라 대사증후군과 같은 이유로 순전이율만 나온다",
    "hyperuricemia": "가역. 식이·요산저하제로 되돌아가고 사망연계 검증을 하지 않았다",
    "liver_enzyme_high": "가역. ALT 는 체중·음주가 바뀌면 몇 달 만에 정상화된다",
    # 가장 가역적인 축이다. 체중 감량·금연·활동량으로 몇 달 안에 내려가고, 급성
    # 구간(>10)은 라벨에서 아예 뺐다. "한 번 생기면 없어지지 않는다" 는 illness-death
    # 의 전제와 정반대라 누적 발병 곡선이 뜻을 갖지 못한다.
    "inflammation": "가역. 체중·활동·금연으로 몇 달 안에 내려간다",
}
