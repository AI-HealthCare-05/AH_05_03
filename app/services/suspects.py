...
def _by_verdict(
    ...
    """**카드 등급이 1차 정렬, 같은 등급 안에서는 확률 내림차순이다.**

    등급을 1차로 두는 것은 그대로다 — 패널 제목이 "급한 순" 이므로 '주의' 카드가
    '정상' 카드 밑으로 내려가면 안 된다. `known` 도 보지 않는다: 확진이라도 가장
    급하면 가장 급한 것이다.

    **2026-09-08, 같은 등급 안의 순서를 확률로 바꿨다(사용자 결정).** 그 전에는
    `신호강도 → settled → 확률` 이었고, 그래서 화면에 9% · 2% · 18% 처럼 확률이
    뒤죽박죽으로 찍혔다. 정렬 근거인 `settled` 와 신호강도는 화면에 안 나오니
    사용자에게는 순서가 고장난 것으로만 보인다. 그 지적을 받아들여 눈에 보이는 값을
    정렬 기준으로 삼는다.

    **버리는 것이 둘 있다. 되돌릴 사람이 알고 되돌리라고 적어 둔다.**

    하나 — 라벨을 만드는 검사값은 그 질환의 ML 입력에서 차단된다
    (`modeling/targets.py` 의 `blocked`). `low_hdl` 모델은 HDL 을 **볼 수 없어서**
    HDL 81 인 사람도 확률이 낮게 나오지 않는다. 이제 그 카드가 "기준 안에 있어요"
    라고 답한 채로 자리채움 1 등에 오를 수 있다. 예전 `settled` 키가 막던 것이다.

    둘 — 절대 확률은 유병률을 그대로 따라간다(이상지질 47% · 고혈압 42%). 같은
    등급 안에서는 이 둘이 거의 언제나 위에 오므로 그만큼 개인화가 준다. 동년배배수와
    근거가중은 점수(`row[0]`)에 남아 있지만 이제 확률이 같을 때만 쓰인다.
    """
    scored: Scored = []
    ...
    scored.sort(
        key=lambda row: (
            ...
            # 화면이 보여주는 값으로 줄 세운다. 확률이 없는 카드는 0 으로 취급돼 뒤로 간다.
            -(row[1].get("probability") or 0.0),
            # 확률이 같을 때만 신호강도(근거가중 × 동년배배수)가 동점을 푼다.
            -row[0],
        )
    )
    return scored
