...
def session_revoked(ctx: ToolPolicyContext) -> bool:
    if not ctx.pin_session_valid:
        return True
    if ctx.session_epoch is not None and ctx.current_session_epoch is not None:
        if ctx.session_epoch != ctx.current_session_epoch:
            return True
    if ctx.pin_actor_id is not None and ctx.current_pin_actor_id is not None:
        if ctx.pin_actor_id != ctx.current_pin_actor_id:
            return True
    return False


def _actor_profile_id(ctx: ToolPolicyContext) -> UUID:
    return ctx.actor_profile_id or ctx.active_profile_id
...
def can_read_records(ctx: ToolPolicyContext, target_profile_id: UUID) -> bool:
    ...
    if target_profile_id != ctx.active_profile_id:
        return False
    self_target = _actor_profile_id(ctx) == ctx.active_profile_id
...
def _public_lookup_names() -> frozenset[str]:
    return frozenset(
        spec.name
        for spec in TOOLS_BY_NAME.values()
        if spec.enabled and spec.exposure != "not_agent_callable" and spec.name not in HEALTH_RECORD_TOOLS
    )
...
def allowed_tools_for(ctx: ToolPolicyContext) -> frozenset[str]:
    if session_revoked(ctx):
        return frozenset()
    names = set(_public_lookup_names())
    if can_read_records(ctx, ctx.active_profile_id):
        names.add("query_health_records")
        if ctx.actor_role is not MemberRole.RESTRICTED:
...
def authorize_tool(
    ...
    *,
    ...
    if for_model and spec.exposure != "model_selectable":
        raise ToolPolicyError(TOOL_NOT_MODEL_SELECTABLE)
