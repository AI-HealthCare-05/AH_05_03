...
class HealthAssistantLlmResponse(BaseModel):
    ...
    suggested_quick_replies: list[str] = Field(
        default_factory=list, description="사용자가 누르기 편한 추천 빠른 답변 목록. 주의: 약국/병원/응급실 방문을 권유했다면 반드시 '주변 약국 검색해줘' 또는 '근처 병원 찾아줘'와 같은 시설 검색 유도어를 포함하고, '약국 방문할게요'와 같이 모호한 표현은 추천어로 쓰지 마세요."
    )
