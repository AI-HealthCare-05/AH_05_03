...
class TestChatSessionsApi:
    ...
    async def test_core_memory_accumulates_after_10_messages(self, authorized_client: AsyncClient, monkeypatch) -> None:
        ...
        create_res = await authorized_client.post(
            "/api/v1/chat-sessions",
            json={"profile_id": "profile-alpha", "title": "핵심 기억 테스트"},
        )
        session_id = create_res.json()["data"]["id"]
        ...
        # 아직 core_memory는 null이어야 함
        get_res = await authorized_client.get(f"/api/v1/chat-sessions/{session_id}")
        assert get_res.json()["data"]["core_memory"] is None

        # 4. 10번째 메시지 전송 (이때 _update_core_memory가 트리거됨)
        res = await authorized_client.post(
            f"/api/v1/chat-sessions/{session_id}/messages",
