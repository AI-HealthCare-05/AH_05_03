...
class TestRefreshAPI:
    ...
    async def test_new_access_token_works(self, client: AsyncClient) -> None:
        tokens = await _login(client, "refresh_works@example.com")
    ...
    async def test_reused_refresh_token_kills_the_whole_family(self, client: AsyncClient) -> None:
        ...
        second = await client.post("/api/v1/auth/refresh", json={"refresh_token": new_refresh_token})
    ...
    async def test_access_token_rejected_as_refresh(self, client: AsyncClient) -> None:
        ...
        tokens = await _login(client, "wrongtype@example.com")
    ...
    async def test_missing_refresh_token_is_validation_error(self, client: AsyncClient) -> None:
