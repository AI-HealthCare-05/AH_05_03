...
def build_system_instruction(
    ...
    if profile_context:
        ...
        if ctx.previous_conversations_summary:
            details.append(
                f"이전 대화 세션 기록 (대화리스트 기반 사용자 건강 기억 및 맥락):\n{ctx.previous_conversations_summary}"
            )
        if details:
...
6. 이전 대화 맥락 및 지속적 건강 상태(임신, 수유, 만성질환 등) 연계 안내 (매우 중요):
   - 사용자가 현재 대화(동일 세션 내 앞선 턴) 또는 이전 대화 세션(대화리스트의 과거 세션들)에서 임신, 수유, 만성질환(고혈압, 당뇨 등), 알레르기, 특정 약물 복용 등을 언급한 적이 있다면:
   ...
   - [예시]: 사용자가 이전 대화에서 "임신 중"이라고 밝혔고 이번에 "타이레놀 1알 먹었어"라고 보고한 경우:
     * 복약 기록은 정상 저장(`intent: "record_medication"`, `medication_draft` 작성, `auto_save=true`)합니다.
     * `assistant_message`에서는 "타이레놀 1알 복용 기록을 저장했습니다. 이전 대화에서 임신 중이라고 말씀해 주셨는데, 타이레놀(아세트아미노펜)은 임신 중 발열이나 통증 시 비교적 안전하게 사용되는 진통제이지만, 임의로 과다 복용하거나 장기 복용하지 않도록 주의하시고 증상이 지속되면 반드시 담당 산부인과 주치의와 상의하세요."와 같이 임신 상태와 약물을 결합하여 친절하고 안전하게 안내하세요.
