...
14. **프런트엔드 작업 시 `DESIGN.md` 단일 진실 원천(SSOT) 준수.** 정본은 Stitch Violet Capsule(Aubergine `#3c315b`, Ghost Lavender `#e2dffe`). 예전 Primary Blue `#1d4fb8`·Inter/Pretendard 32/24/18 스케일은 `DESIGN.ieobom.md` 백업이다. 임의 폰트/인라인 색/새 키 컬러 금지. 타이포는 DM Sans + Noto Sans KR(11/13/15/16/20/24/30px), 스켈레톤 UI, 터치 44px.
