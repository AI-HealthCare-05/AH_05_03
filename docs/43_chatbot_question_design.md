...
- 판정기는 필요한 근거를 `health_knowledge`·`medication`·`food_nutrition`·`outdoor`·`facility`·`health_records`의 복수 목록으로 반환한다. 원칙적으로 코드가 전부 충족됐는지 검사한다. 예외적으로 민감한 개인 활동 질문에서 요구한 의료 근거가 모두 있고 **빠진 근거가 `outdoor`뿐이면** 일반 건강정보의 부분 답변을 허용한다. 날씨 결과가 질환 기준을 대체할 수 없고, 음식·시설·기록 등 다른 필수 근거의 누락도 이 예외로 덮지 않는다.
- 이 부분 답변의 현재 날씨 추측 금지는 답변 프롬프트가 지시한다. 자연어 문장마다 근거 종류를 기계적으로 증명하는 단계는 아직 없으므로, 이 예외를 개인별 운동 허가의 안전성 보증으로 해석하지 않는다.
