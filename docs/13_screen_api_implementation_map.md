...
| 계정·프로필 연결 | `/api/v1/profile-links` | 구현됨 | `/account` 초대 수락·기존 로컬 프로필 연결 UI |
...
| `/account` | 가입·로그인·가정·초대·프로필 연결 | `ServerApiClient` 연결, 건강정보 전송 없음 |
...
1. 목표 OpenAPI와 실제 초대 취소·멱등성·`If-Match` 계약을 하나로 통일한다.
2. OPFS 문서 저장의 용량 사전 검사·해시 검증·staging 고아 정리를 구현한다.
3. 대용량 백업을 스트리밍 컨테이너로 전환하고 이전 백업 버전 변환기를 추가한다.
4. OCR 모델 자산을 자체 호스팅해 초기 실행 시 외부 CDN 의존을 제거한다.
5. 프로필 비교·충돌 해결·병합·되돌리기 UI를 연결한다.
