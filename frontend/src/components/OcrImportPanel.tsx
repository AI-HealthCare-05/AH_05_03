...
export function OcrImportPanel() {
  ...
  const recognize = async () => {
    ...
    try {
      ...
      if (source.documentId) {
        ...
        setMessage("서류 원본과 수정 초안을 암호화해 저장했습니다.");
      }
    } catch (reason) { setError(reason instanceof Error ? reason.message : "서류 분석에 실패했습니다."); }
  ...
  const confirm = async () => {
    ...
    setStatus("confirmed"); setError(""); setMessage("수정한 서류 분석 결과를 확정했습니다.");
  };
  ...
  const connectRecord = async () => {
    ...
    const result = await recordService.create({
      ...
      payload: { type: "health_screening", screeningName: source.file.name, summary: summary || "확정된 서류 분석 내용 없음" }, duplicatePolicy: "reject",
    });
    if (!result.ok) return setError(result.error.message);
  ...
  return <>
    <button className="ocr-launcher" onClick={() => setOpen(true)}>건강 서류 분석</button>
    {open && <div className="modal-backdrop ocr-backdrop"><section className="ocr-panel" role="dialog" aria-modal="true" aria-labelledby="ocr-title">
      <header><div><p className="eyebrow">사용자 확인 단계</p><h2 id="ocr-title">건강 서류 분석 결과 확인</h2></div><button onClick={() => setOpen(false)} aria-label="닫기">×</button></header>
      <div className="ocr-warning"><strong>서류 분석은 의료 판단을 하지 않아요.</strong><span>확인된 글자와 표를 직접 고친 뒤 확정해야 건강기록으로 연결됩니다.</span></div>
      <label className="file-drop">JPEG 또는 PNG 선택<input type="file" accept="image/jpeg,image/png" onChange={(event) => { const file = event.target.files?.[0]; setSource(file ? { file } : null); setDraft(null); setStatus(null); setMessage(""); }} /><small>{source ? `${source.file.name} · ${(source.file.size / 1024 / 1024).toFixed(1)}MB` : "원본 서류 화면에서 내용 읽기를 누르면 저장·연결까지 가능합니다."}</small></label>
      <button className="primary ocr-run" disabled={!source || loading} onClick={() => void recognize()}>{loading ? "글자를 읽는 중…" : "서류 분석하기"}</button>
      {error && <div className="form-error" role="alert">{error}</div>}{message && <div className="form-success">{message}</div>}
      {draft && <div className="ocr-result"><div className="raw-badge">{status === "confirmed" ? "확정됨 · 사용자 확인" : "수정 필요"}</div><h3>일반 텍스트</h3><textarea className="ocr-text-editor" value={draft.text} onChange={(event) => { setDraft({ ...draft, text: event.target.value }); setStatus("draft"); setMessage(""); }} /><h3>검사 항목 확인</h3>{(draft.examItems ?? []).length === 0 && <p className="ocr-inline-note">자동으로 나눌 수 있는 검사 항목이 없습니다. 아래 원본 표를 확인해 주세요.</p>}{(draft.examItems ?? []).length > 0 && <div className="ocr-table-wrap"><table><thead><tr><th>검사항목</th><th>결과값</th><th>단위</th><th>판정</th></tr></thead><tbody>{draft.examItems!.map((item, index) => <tr key={index}>{(["testName", "value", "unit", "judgment"] as Array<keyof OcrExamItem>).map((field) => <td key={field}><input value={item[field]} aria-label={`${index + 1}번째 ${field}`} onChange={(event) => updateExamItem(index, field, event.target.value)} /></td>)}</tr>)}</tbody></table></div>}<details><summary>분석한 원본 표 확인</summary>{draft.tables.length === 0 && <p>확인된 원본 표가 없습니다.</p>}{draft.tables.map((table, tableIndex) => <div className="ocr-table-wrap" key={table.table_index}><table><tbody>{table.rows.map((row, rowIndex) => <tr key={rowIndex}>{row.map((cell, cellIndex) => <td key={cellIndex}><input value={cell} aria-label={`표 ${tableIndex + 1} 행 ${rowIndex + 1} 열 ${cellIndex + 1}`} onChange={(event) => updateCell(tableIndex, rowIndex, cellIndex, event.target.value)} /></td>)}</tr>)}</tbody></table></div>)}</details>
