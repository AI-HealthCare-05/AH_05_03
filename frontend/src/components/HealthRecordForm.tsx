...
type Category = "metric" | "care" | "pain" | "lifestyle" | "other";
interface ProfileOption { id: string; name: string; relationship: string }
interface Props { householdId: string; profiles: ProfileOption[]; onShowHistory: () => void }
...
function payloadFromForm(category: Category, data: FormData): HealthPayload {
  ...
  if (category === "pain") return { type: "pain", bodyArea: text(data, "bodyArea"), intensity: Number(data.get("intensity")), sensation: text(data, "sensation") || undefined, onsetAt: text(data, "onsetDate") ? atLocalNoon(text(data, "onsetDate")) : undefined, aggravatingFactors: text(data, "aggravatingFactors") || undefined, note };
...
const categories: Array<[Category, string, string]> = [
  ...
  ["pain", "통증 기록", "부위·강도·양상 기록"],
  ["lifestyle", "생활 기록", "걸음 수·거리·시간"],
  ["other", "전문 검사·기타", "검사 또는 관찰 요약"],
];

export function HealthRecordForm({ householdId, profiles, onShowHistory }: Props) {
  ...
  const changeCategory = (next: Category) => {
    setCategory(next);
    setSubtype(next === "metric" ? "blood_pressure" : next === "care" ? "vaccination" : next === "pain" ? "pain" : next === "lifestyle" ? "walking" : "note");
    setError(null);
  };
  ...
  return <main className="page-shell">
    <header className="page-header"><button className="back" type="button" onClick={onShowHistory}>← 건강 이력</button><div><p className="eyebrow">로컬 건강 기록</p><h1>건강기록 작성</h1><p>가족의 기록을 직접 입력하고 이 브라우저에 안전하게 보관하세요.</p></div></header>
    ...
    {!saved && <form onSubmit={onSubmit} noValidate>
      ...
      <section className="section"><div className="section-heading"><span>2</span><div><h2>기록 유형</h2><p>선택한 유형에 맞는 항목만 보여드려요.</p></div></div><div className="category-grid">{categories.map(([id, label, help]) => <button type="button" key={id} className={id === category ? "category active" : "category"} onClick={() => changeCategory(id)}><i>{id === "metric" ? "↗" : id === "care" ? "+" : id === "pain" ? "!" : id === "lifestyle" ? "◷" : "□"}</i><strong>{label}</strong><small>{help}</small></button>)}</div></section>
      {category === "pain" && <section className="section form-section pain-section"><div className="section-heading"><span>통증</span><div><h2>통증 정보</h2><p>느낀 증상을 있는 그대로 기록해 주세요.</p></div></div><div className="form-grid"><label>통증 부위<input name="bodyArea" placeholder="예: 오른쪽 윗배" required />{fieldError("bodyArea") && <b>{fieldError("bodyArea")}</b>}</label><label>통증 강도 (0~10)<input name="intensity" type="number" min="0" max="10" defaultValue="5" required />{fieldError("intensity") && <b>{fieldError("intensity")}</b>}</label><label>통증 양상<input name="sensation" placeholder="예: 찌르듯이, 욱신거림" /></label><label>처음 시작한 날<input name="onsetDate" type="date" /></label><label className="full">심해지는 상황<input name="aggravatingFactors" placeholder="예: 식사 후, 움직일 때" /></label></div></section>}
