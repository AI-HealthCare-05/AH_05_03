...
export class OcrResultService {
  async saveDraft(documentId: string, content: OcrContent): Promise<LocalResult<StoredOcrResult>> {
    ...
    } catch { return { ok: false, error: { code: "STORAGE_PERMISSION_DENIED", message: "OCR 초안을 저장하지 못했습니다.", retryable: true } }; }
  }
  ...
  async confirm(resultId: string, content: OcrContent): Promise<LocalResult<StoredOcrResult>> {
    try {
      ...
      if (!existing) return { ok: false, error: { code: "NOT_FOUND", message: "OCR 초안을 찾을 수 없습니다.", retryable: false } };
    ...
    } catch { return { ok: false, error: { code: "ENCRYPTION_FAILED", message: "OCR 확정 결과를 저장하지 못했습니다.", retryable: true } }; }
  }
}
