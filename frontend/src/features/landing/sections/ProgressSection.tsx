...
export function ProgressSection() {
  ...
  return (
    <section className="ln-progress ln-scroll" ref={ref} aria-labelledby="ln-progress-title">
      <div className="ln-sticky ln-progress-stage">
        <header className="ln-section-head">
          <p className="ln-eyebrow">기록과 변화</p>
          <h2 id="ln-progress-title" className="ln-headline">
            작은 기록이,
            <br />
            건강의 변화를 만듭니다.
          </h2>
        ...
        <div className="ln-progress-body">
          <div className="ln-progress-chart">
            <div className="ln-progress-readout">
              <p className="ln-progress-week">{week.label}</p>
              <p className="ln-progress-metric">
                {week.metricValue}
                <small>{LANDING_METRIC_UNIT}</small>
              </p>
              <p className="ln-progress-caption">
                {LANDING_METRIC_LABEL} · 챌린지 수행률 {week.rate}%
              </p>
            </div>

            <svg viewBox={`0 0 ${VIEW_WIDTH} ${VIEW_HEIGHT}`} role="img" aria-label="4주간 수행률과 공복혈당 추이">
              <g className="ln-progress-grid" aria-hidden="true">
                {LANDING_WEEKS.map((item, itemIndex) => {
                  ...
                  return (
                    <line
                      ...
                      y1={PAD_Y - 14}
                      x2={x}
                      y2={VIEW_HEIGHT - PAD_Y + 14}
                      data-done={itemIndex <= index}
                    />
                  );
                })}
              </g>
              <polyline className="ln-progress-line ln-progress-line-rate" points={ratePoints} pathLength={1} />
              <polyline className="ln-progress-line ln-progress-line-metric" points={metricPoints} pathLength={1} />
              <g className="ln-progress-labels" aria-hidden="true">
                {LANDING_WEEKS.map((item, itemIndex) => {
                  const x = PAD_X + ((VIEW_WIDTH - PAD_X * 2) / (LANDING_WEEKS.length - 1)) * itemIndex;
                  return (
            ...
            {/* 두 선은 축이 다르다. 단위를 legend 에 적지 않으면 "혈당이 올라간 것"
                으로 읽힐 수 있다. */}
            <ul className="ln-progress-legend" aria-hidden="true">
          ...
          <figure className="ln-progress-garden">
            <Tree stage={TREE_STAGES[index] ?? "sprout"} size={144} label={`${week.label} 정원`} />
            <figcaption>
              기록을 남긴 만큼 자라는 정원.
              <br />
        ...
        <p className="ln-sr-only">
          {LANDING_WEEKS.map(
            (item) => `${item.label} 수행률 ${item.rate}%, ${LANDING_METRIC_LABEL} ${item.metricValue} ${LANDING_METRIC_UNIT}`,
          ).join(". ")}
        </p>
      </div>
    </section>
