 ...
 *
 * 그룹 순서는 `/api/demo` 가 이미 쓰던 순서를 따랐다(기본 → 혈압 → 혈당 → 지질 →
 * 간·신장·혈액 → 생활습관 → 진단 이력). 검진결과지에 인쇄된 순서와 비슷해서
 * 위에서 아래로 옮겨 적을 수 있다.
 */
...
export const FIELD_GROUPS: FieldGroup[] = [
  {
    key: "basic",
    title: "기본",
    note: "이 다섯만 채우면 판정이 나온다. 나머지는 넣을수록 답하는 칸이 늘어난다.",
    fields: [
  ...
  },
  {
    key: "bp",
    title: "혈압",
    note: "넣으면 고혈압 판정이 ML 추정에서 규칙 엔진으로 넘어간다.",
    fields: [
  ...
  },
  {
    ...
    fields: [
      ...
      { name: "non_hdl_c", label: "비HDL", kind: "number", unit: "mg/dL", min: 1, max: 1000, hint: "비우면 총-HDL 로 계산" },
    ],
  },
  {
    key: "organ",
    title: "간 · 신장 · 혈액",
    note: "크레아티닌은 신기능(eGFR), AST·ALT·γ-GTP 는 간, 혈색소는 빈혈 판정을 연다.",
    fields: [
  ...
  },
  {
    key: "history",
    title: "진단 이력",
    note: "이미 진단받은 질환은 위험을 세지 않는다 — 진단자에게 '위험 높음'은 아무 정보가 아니다.",
    fields: [
