...
BASIC_FEATURES: tuple[str, ...] = (
    ...
    "waist_cm",
    "smoking_status",
    # `difficulty_walking` 을 2026-09-03 에 뺐다. 두 가지가 겹쳤다.
    #
    # ① **홀드아웃 주기(2021_2023)에 한 행도 없다.** PFQ 파일이 그 주기에 미발간이라
    #    커버리지가 정확히 0% 다. 학습에서 무엇을 배웠든 평가에서는 전부 결측 대치되므로
    #    이 특징의 기여는 구조적으로 측정 불가능했다.
    # ② 그런데도 서빙 번들 20개에 들어가 있었고 온보딩이 사용자에게 묻고 있었다.
    #
    # 뺐을 때 손실이 있는지 두 홀드아웃에서 재 봤다(`experiment_features.py`).
    # 전체에서 유의한 손실 없음, **미진단자 6/6 칸에서 오히려 양수**(부호검정 p=0.031).
    # 자세히는 `docs/42_ml_evaluation_strategy.md` §5.
    "alcohol_days_per_year",
    "moderate_min_per_week",
    "vigorous_min_per_week",
    "sedentary_min_per_day",
    "sleep_hours",
    # `education_level` 을 2026-09-03 에 뺐다. 이건 성능이 아니라 **제품 판단**이다.
    #
    # 건강 앱이 학력을 묻는 것이 사용자에게 어떻게 읽히는지가 이유고, 정확도로
    # 치르는 값은 먼저 재고 받아들였다. 200 명 표본에서 답한 경우와 안 물어본
    # 경우를 비교하면 확률 평균 |Δ| 0.005, 등급이 바뀐 칸 1.5%(29/2000) 다.
    # 여섯 질환은 사실상 0 이고 지방간만 컸다(평균 0.012, 최대 0.139).
    #
    # 폼에서만 빼면 모든 사용자가 중앙값 대치를 받는다 — 그건 값을 안 쓰는 게
    # 아니라 **모두에게 같은 가짜 값을 주는 것**이라, 학습에서도 같이 뺀다.
    "sbp",
    "dbp",
)
